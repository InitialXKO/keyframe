...
export function axisAngle(ax, a) {
...
function alignY(d) {
    ...
    const y = d[1] / l;
    if (y < -0.9999)
        return axisAngle([0, 0, 1], Math.PI);
    const s = Math.hypot(d[0], d[2]) / l;
    if (s < 1e-8)
        return [0, 0, 0, 1];
    const ax = [-d[2] / l / s, 0, d[0] / l / s];
    return axisAngle(ax, Math.acos(Math.max(-1, Math.min(1, y))));
}
// ---------- 运动学求解（动画系统的每帧位姿反算） ----------
export function solveKinematics(sc, t) {
    ...
    let phi = 0;
    if (sc.kin)
        phi = t * sc.kin.freq * Math.PI * 2;
    const c = sc.kin ? sc.kin.c : [0, 0, 0];
    const R = sc.kin ? sc.kin.R : 0;
    const L = sc.kin ? sc.kin.L : 1;
    // 曲柄销位置（曲柄平面 XY，轴 Z）
    const pin = [c[0] + R * Math.cos(phi), c[1] + R * Math.sin(phi), c[2]];
    // 滑块约束：x=c[0] 竖直线，销到滑块销距离恒 L（曲柄滑块闭环方程）
    const py = c[1] + R * Math.sin(phi) + Math.sqrt(Math.max(L * L - R * R * Math.cos(phi) ** 2, 1e-6));
    const piston = [c[0], py, c[2]];
    const rodDir = [piston[0] - pin[0], piston[1] - pin[1], piston[2] - pin[2]];
    for (const pr of sc.prims) {
        const q = pr.quat ?? [0, 0, 0, 1];
        let p = pr.pos;
        switch (pr.anim) {
            case 'pin':
                p = pin;
...
const RX90 = axisAngle([1, 0, 0], Math.PI / 2);
function pistonAssembly() {
    const P = (o) => o;
    const B = (op, trans, radius = 0, extra = {}) => ({ op, trans, radius, ...extra });
    const prims = [
...
export const PRESETS = [pistonAssembly(), pipeJunction()];
// ---------- 构造指令校验（编辑器 JSON → 引擎的安全入口） ----------
// 输入 = 几何基元列表 + 结合与过渡描述表；任何非法指令在进入引擎前被拦截并给出定位。
const PRIM_TYPES = ['box', 'sphere', 'cyl', 'torus', 'capsule', 'boltring'];
const PRIM_ARITY = { box: 3, sphere: 1, cyl: 2, torus: 2, capsule: 2, boltring: 4 };
const OP_TYPES = ['union', 'sub', 'inter'];
const TRANS_TYPES = ['sharp', 'fillet', 'chamfer', 'wave', 'lap'];
const ANIM_KINDS = ['disc', 'pin', 'bolts', 'rod', 'piston', 'none'];
const isNum = (v) => typeof v === 'number' && Number.isFinite(v);
const numArr = (v, n) => Array.isArray(v) && v.length === n && v.every(isNum);
export function validateScene(raw) {
    ...
    const clusterRaw = (o.cluster ?? o.集群 ?? null);
    if (typeof o.name !== 'string' || !o.name.trim())
        return { ok: false, error: 'name 必须是非空字符串' };
    if (!Array.isArray(primsRaw) || primsRaw.length === 0)
        return { ok: false, error: 'prims 必须是非空数组' };
    if (primsRaw.length > 16)
        return { ok: false, error: `prims 数量 ${primsRaw.length} 超过上限 16` };
    const prims = [];
    for (let i = 0; i < primsRaw.length; i++) {
        const p = primsRaw[i];
        if (!p || typeof p !== 'object')
            return { ok: false, error: `prims[${i}] 必须是对象` };
        if (!PRIM_TYPES.includes(p.type))
            return { ok: false, error: `prims[${i}].type "${String(p.type)}" 不在 ${PRIM_TYPES.join('/')}` };
        const tp = p.type;
        if (!numArr(p.pos, 3))
            return { ok: false, error: `prims[${i}].pos 必须是 3 个有限数字` };
        ...
        if (p.anim !== undefined && p.anim !== null && !ANIM_KINDS.includes(p.anim))
        ...
        prims.push({
            ...
            ...(Array.isArray(p.quat)
                ? { quat: [p.quat[0], p.quat[1], p.quat[2], p.quat[3]] }
                : {}),
            ...(p.anim ? { anim: p.anim } : {}),
        });
    }
    ...
    for (let i = 0; i < bondsRaw.length; i++) {
        ...
        if (!b || typeof b !== 'object')
            return { ok: false, error: `bonds[${i}] 必须是对象` };
        if (!OP_TYPES.includes(b.op))
            return { ok: false, error: `bonds[${i}].op "${String(b.op)}" 无效` };
        if (!TRANS_TYPES.includes(b.trans))
        ...
        if (b.k0 !== undefined && !isNum(b.k0))
            return { ok: false, error: `bonds[${i}].k0 必须是数字` };
        if (b.k1 !== undefined && !isNum(b.k1))
            return { ok: false, error: `bonds[${i}].k1 必须是数字` };
        bonds.push({
            op: b.op,
            trans: b.trans,
            radius: b.radius ?? 0,
            waveAmp: b.waveAmp ?? 0,
            waveFreq: b.waveFreq ?? 0,
            chamAxis: b.chamAxis ?? 1,
            k0: b.k0 ?? 0,
            k1: b.k1 ?? 0,
        });
    }
    if (!numArr(boundC, 3))
        return { ok: false, error: 'boundC 必须是 3 个数字' };
    if (!isNum(boundR) || boundR <= 0)
        return { ok: false, error: 'boundR 必须是正数' };
    let kin = null;
    ...
    if (!isNum(turntable) || turntable < 0)
        return { ok: false, error: 'turntable 必须 ≥ 0' };
    let cluster = null;
