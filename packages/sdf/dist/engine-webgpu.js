...
// ============================================================
import { PRESETS, LABELS, solveKinematics, axisAngle, qmul } from './scene.js';
import { WGSL } from './shader-wgsl.js';
import { cpuProbe, makePickCtx } from './eval-cpu.js';
import { MAXP, packStatic, packPoses, packBlock, sceneSeed } from './pack.js';
import { solvePoses, initKeyframeBridge, disposeKeyframeBridge, isKeyframeBridgeActive } from './keyframe-bridge.js';
const TANF = 0.3839;
function qrot(q, v) {
    const [qx, qy, qz, qw] = q;
    const c1 = [qy * v[2] - qz * v[1], qz * v[0] - qx * v[2], qx * v[1] - qy * v[0]];
    const w1 = [c1[0] + qw * v[0], c1[1] + qw * v[1], c1[2] + qw * v[2]];
    const c2 = [qy * w1[2] - qz * w1[1], qz * w1[0] - qx * w1[2], qx * w1[1] - qy * w1[0]];
...
export class SdfWebGPUEngine {
    ...
    sc;
    st;
    P1 = new Float32Array(MAXP * 4);
    P3 = new Float32Array(MAXP * 4);
    waveMax = 0;
    ...
    clusterCell = 5.5;
    clusterSpread = 0.6;
    radius0 = new Float32Array(MAXP); // 过渡半径基线（脉动调制用）
    constructor(canvas) {
        this.canvas = canvas;
    }
    ...
    static async createInner(canvas, onError) {
        const nav = navigator;
        if (!nav.gpu)
            throw new Error('navigator.gpu 不可用（浏览器无 WebGPU 入口）');
        const adapter = await nav.gpu.requestAdapter();
        if (!adapter)
            throw new Error('WebGPU adapter 枚举失败（无可用图形后端）');
        ...
        try {
            ...
            eng.adapterName = 'WebGPU 适配器';
        }
        device.addEventListener?.('uncapturederror', (ev) => {
            eng.onError?.(String(ev?.error?.message || ev));
        });
        device.lost?.then((dinfo) => {
            if (dinfo?.reason !== 'destroyed')
                eng.onError?.('WebGPU 设备丢失: ' + (dinfo?.reason || ''));
        });
        ...
        if (module.getCompilationInfo) {
            ...
            if (errs.length) {
                throw new Error('WGSL 编译失败: ' + errs.map((m) => `L${m.lineNum}:${m.linePos} ${m.message}`).join(' | '));
            }
        }
        const mkPipeline = async (targetFormat, tag) => {
            ...
            if (perr)
                throw new Error(`WebGPU 管线(${tag})创建失败: ${perr.message}`);
            return p;
        };
        ...
        if (!ctx)
            throw new Error('canvas webgpu 上下文获取失败（画布已被其他 API 占用）');
        ctx.configure({ device, format, alphaMode: 'premultiplied' });
        eng.ctx = ctx;
        eng.ready = true;
        return eng;
    }
    setScene(idx) { this.loadScene(PRESETS[idx]); }
    loadScene(raw) {
        this.sc = JSON.parse(JSON.stringify(raw));
        this.st = packStatic(this.sc);
        this.waveMax = this.st.waveMax;
        this.radius0.set(this.st.B0.filter((_, i) => i % 4 === 2));
    ...
    async setKeyframeConfig(config) {
        return await initKeyframeBridge(this.sc, config);
    }
    async enableKeyframeEngine(config) {
        return await initKeyframeBridge(this.sc, config);
    }
    disableKeyframeBridge() {
        disposeKeyframeBridge();
    }
    disableKeyframeEngine() {
        disposeKeyframeBridge();
    }
    get isKeyframeActive() {
    ...
    frame(nowMs) {
        ...
        if (!this.paused) {
            ...
            this.wearAge += dt * this.speed;
        }
        if (this.wearKey)
            this.wearAges.set(this.wearKey, this.wearAge);
        if (this.orbit && !this.paused)
            this.yaw += dt * this.speed * 0.28;
        this.fpsEma = this.fpsEma * 0.92 + (1 / Math.max(dt, 1e-3)) * 0.08;
        this.fps = this.fpsEma;
        if (++this.frames % 30 === 0) {
            if (this.fpsEma < 13 && this.renderScale > 0.35)
                this.renderScale *= 0.85;
            else if (this.fpsEma > 38 && this.renderScale < 1.0)
                this.renderScale = Math.min(1.0, this.renderScale * 1.12);
        }
        ...
        for (let i = 0; i < MAXP; i++)
            this.st.B0[i * 4 + 2] = this.radius0[i] * km;
        packPoses(this.sc, this.P1, this.P3);
        const cl = [this.clusterOn ? 1 : 0, this.clusterCell, this.clusterSpread, this.timeSec];
        const block = packBlock([w, h], this.camQ(), this.dist, this.zoom, this.waveMax, Math.min(this.sc.prims.length, MAXP), [this.sc.boundC[0], this.sc.boundC[1], this.sc.boundC[2], this.sc.boundR], 0, [0, 0, -1], [this.wearAge, sceneSeed(this.sc.name)], this.st, this.P1, this.P3, cl);
        this.draw(block);
    }
    async probe(ndcX, ndcY) {
        if (!this.ready)
            return null;
        const aspect = (this.canvas.width || 1) / (this.canvas.height || 1);
        const rd = norm3([ndcX * TANF * aspect, ndcY * TANF, -1]);
        try {
            if (!this.probePipeline)
                throw new Error('probe pipeline 不可用');
            const read = async (mode) => {
                const cl = [this.clusterOn ? 1 : 0, this.clusterCell, this.clusterSpread, this.timeSec];
                const block = packBlock([this.canvas.width, this.canvas.height], this.camQ(), this.dist, this.zoom, this.waveMax, Math.min(this.sc.prims.length, MAXP), [this.sc.boundC[0], this.sc.boundC[1], this.sc.boundC[2], this.sc.boundR], mode, rd, [this.wearAge, sceneSeed(this.sc.name)], this.st, this.P1, this.P3, cl);
                const d = this.device;
                d.queue.writeBuffer(this.ubuf, 0, block);
                const enc = d.createCommandEncoder();
                const pass = enc.beginRenderPass({
                ...
                pass.draw(3);
                pass.end();
                enc.copyTextureToBuffer({ texture: this.probeTex }, { buffer: this.probeBuf, bytesPerRow: 256 }, [1, 1, 1]);
                d.queue.submit([enc.finish()]);
            ...
            const tCam = m1[0];
            const pCam = [rd[0] * tCam, rd[1] * tCam, rd[2] * tCam + this.dist];
            const pw = qrotInv(this.camQ(), pCam).map((v) => v * this.zoom);
            const nCam = [m2[0], m2[1], m2[2]];
            ...
            const ctx = makePickCtx(this.sc, this.st, this.P1, this.P3, this.camQ(), this.dist, this.zoom, this.canvas.height || 1, { on: this.clusterOn, cellSize: this.clusterCell, spreadAmp: this.clusterSpread, time: this.timeSec });
    ...
    async pickAt(clientX, clientY) {
        ...
        if (!p || !p.hit)
            return null;
        const nm = (i) => (i >= 0 && i < LABELS.length ? LABELS[i] : '未知');
        let label = nm(p.labelA);
        if (p.labelB >= 0 && p.labelB !== p.labelA)
            label += ` ⇄ ${nm(p.labelB)}（混合 ${(p.w * 100).toFixed(0)}%）`;
        return {
            hit: true,
            source: 'sdf',
            point: p.point,
    ...
    dispose() {
        ...
        try {
            this.probeBuf?.destroy?.();
        }
        catch { /* 忽略 */ }
        try {
            this.probeTex?.destroy?.();
        }
        catch { /* 忽略 */ }
        try {
            this.ubuf?.destroy?.();
