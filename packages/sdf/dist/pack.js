...
const TYPE = { box: 0, sphere: 1, cyl: 2, torus: 3, capsule: 4, boltring: 5 };
const OP = { union: 0, sub: 1, inter: 2 };
const TT = { sharp: 0, fillet: 1, chamfer: 2, wave: 3, lap: 4 };
/** 静态数据（基元类型参数 + 结合描述表）—— loadScene 时打包一次 */
export function packStatic(sc) {
    ...
    sc.prims.forEach((pr, i) => {
        ...
        if (pr.type === 'box')
            p2 = [p[0], p[1], p[2], 0];
        else if (pr.type === 'sphere')
            p2 = [p[0], 0, 0, 0];
        else if (pr.type === 'cyl' || pr.type === 'torus' || pr.type === 'capsule')
            p2 = [p[0], p[1], 0, 0];
        else if (pr.type === 'boltring')
            p2 = [p[0], p[1], p[2], p[3]];
        P2.set(p2, i * 4);
    });
    sc.bonds.forEach((b, i) => {
        if (i >= MAXP)
            return;
        B0.set([OP[b.op], TT[b.trans], b.radius ?? 0, b.waveAmp ?? 0], i * 4);
        B1.set([b.waveFreq ?? 0, b.chamAxis ?? 1, b.k0 ?? 0, b.k1 ?? 0], i * 4);
        if (b.trans === 'wave')
            waveMax = Math.max(waveMax, (b.waveAmp ?? 0) * (b.waveFreq ?? 0));
    });
    return { P0, P2, B0, B1, waveMax };
}
/** 每帧位姿（运动学求解结果）写入 P1/P3 */
export function packPoses(sc, P1, P3) {
    sc.prims.forEach((pr, i) => {
        if (i >= MAXP)
            return;
...
export const SUN = [0.5, 0.62, 0.45];
export const SUNCOL = [1.15, 1.1, 1.0];
/** 场景磨损种子（名称 FNV 哈希）—— 同一场景刷新/切后端后痕迹形态一致 */
export function sceneSeed(name) {
    let h = 2166136261 >>> 0;
...
export function packBlock(res, camQ, dist, scale, waveMax, primCount, bound, mode, probeDir, wear, // [服役时长(s), 场景种子]
st, P1, P3, cluster) {
