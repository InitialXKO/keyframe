...
// ============================================================
import { PRESETS, LABELS, solveKinematics } from './scene.js';
...
export class SdfEngine {
    ...
    vao = null;
    sc = PRESETS[0];
    st = packStatic(PRESETS[0]);
    P1 = new Float32Array(MAXP * 4);
    P3 = new Float32Array(MAXP * 4);
    uLoc = {};
    // 视口与交互
    camQ = axisAngle([1, 0, 0], -0.25);
    dist = 3.6;
    zoom = 1.0;
    renderMode = 0; // 0真实 1防穿透 2材质标签 3网格导线 4磨损热力
    showCluster = false; // 集群增殖域开关
    wearSeconds = 0;
    isDragging = false;
    lastMouse = [0, 0];
    animId = 0;
    ...
    async pickAt(clientX, clientY) {
        ...
        const rdNormalized = [rdS[0] / l, rdS[1] / l, rdS[2] / l];
        const rdW = qrotInv(this.camQ, rdNormalized);
        const zoom = this.zoom;
        const roW = qrotInv(this.camQ, [0, 0, this.dist]).map((v) => v * zoom);
        const pc = Math.min(this.sc.prims.length, MAXP);
        let t = 0.002;
        let hit = false;
        let hitP = [0, 0, 0];
        for (let i = 0; i < 64; i++) {
            const p = [roW[0] + rdW[0] * t, roW[1] + rdW[1] * t, roW[2] + rdW[2] * t];
            let d = 1e9;
            this.sc.prims.forEach((_, idx) => {
                if (idx >= pc)
                    return;
                const qPr = [this.P3[idx * 4], this.P3[idx * 4 + 1], this.P3[idx * 4 + 2], this.P3[idx * 4 + 3]];
                const posPr = [this.P1[idx * 4], this.P1[idx * 4 + 1], this.P1[idx * 4 + 2]];
