...
function smod(x, y) { return x - y * Math.floor(x / y); }
function smin(a, b, k) {
...
function primEval(t, q, P2, i) {
    ...
    switch (t) {
        ...
        default: { // 5 boltring：绕 Z 极坐标周期映射复用单一函数体
            ...
            const qb = [Math.cos(ang) * rr - pw, Math.sin(ang) * rr, q[2]];
...
export function evalSceneMap(p, pc, P0, P1, P2, P3, B0, B1) {
    let acc = 1e9, la = 0, slb = -1, sw = 0, ip = 0;
    for (let i = 0; i < MAXP && i < pc; i++) {
        const q = qrotInv([P3[i * 4], P3[i * 4 + 1], P3[i * 4 + 2], P3[i * 4 + 3]], [p[0] - P1[i * 4], p[1] - P1[i * 4 + 1], p[2] - P1[i * 4 + 2]]);
...
function clusterXform(p, ctx) {
    const cl = ctx.cluster;
    const cs = Math.max(cl.cellSize, 1.5);
    const cell = [Math.floor(p[0] / cs), Math.floor(p[1] / cs), Math.floor(p[2] / cs)];
    const h = hash31(cell);
    const centerW = [(cell[0] + 0.5) * cs, (cell[1] + 0.5) * cs, (cell[2] + 0.5) * cs];
    const pop = 0.5 - 0.5 * Math.cos(cl.time * 0.55 - h[0] * 6.2832);
    const dl = Math.hypot(h[1] - 0.5, h[2] * 0.55 + 0.15, h[0] - 0.5) || 1;
    const off = [(h[1] - 0.5) / dl * cl.spreadAmp * pop, (h[2] * 0.55 + 0.15) / dl * cl.spreadAmp * pop, (h[0] - 0.5) / dl * cl.spreadAmp * pop];
...
export function cpuProbe(rdW, ctx) {
    const zoom = ctx.zoom;
    const roW = qrotInv(ctx.camQ, [0, 0, ctx.dist]).map((v) => v * zoom);
    ...
    if (clustered) {
        ...
        const oc = [roW[0] - ctx.boundC[0], roW[1] - ctx.boundC[1], roW[2] - ctx.boundC[2]];
    ...
    if (!hit)
        return null;
    const pw = [roW[0] + rdW[0] * t, roW[1] + rdW[1] * t, roW[2] + rdW[2] * t];
    ...
    const o = epsw * 0.5773;
    const m = e.map((ev) => mapSceneD([pw[0] + ev[0] * o, pw[1] + ev[1] * o, pw[2] + ev[2] * o], ctx));
    const g = [
...
export function makePickCtx(sc, st, P1, P3, camQ, dist, zoom, resY, cluster) {
    return {
