...
export class WebGPUAdapter {
     ...
     */
    dispatchCompute(device, buffer, time, baseOffset = 0, options) {
     ...
     */
    async readFromBuffer(device, buffer, options) {
        // Layer 3: Device Lost Perception
        this.checkDeviceLost(device);
        ...
        // Layer 2: Out-of-Bounds Protection
        if (buffer && typeof buffer.size === "number" && requiredSize > buffer.size) {
            throw new RangeError(`Buffer overflow: read range ${requiredSize} bytes exceeds GPUBuffer size (${buffer.size} bytes).`);
        }
     ...
     */
    async readInstance(device, buffer, options) {
