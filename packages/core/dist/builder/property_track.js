...
export function interpolateTransform(a, b, factor) {
    return {
        translation: [
...
export class PropertyTrackRegistry {
    ...
    static register(trackName, interpolator) {
        this.registry.set(trackName, interpolator);
    }
    static get(trackName) {
        return this.registry.get(trackName);
    }
    static interpolate(trackName, a, b, factor) {
        const interpolator = this.get(trackName);
        if (interpolator) {
...
PropertyTrackRegistry.register("opacity", (a, b, factor) => a + (b - a) * factor);
PropertyTrackRegistry.register("number", (a, b, factor) => a + (b - a) * factor);
PropertyTrackRegistry.register("color_rgb", (a, b, factor) => {
    return [
        a[0] + (b[0] - a[0]) * factor,
        a[1] + (b[1] - a[1]) * factor,
        a[2] + (b[2] - a[2]) * factor,
