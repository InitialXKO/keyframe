export class DOMAdapter {
     ...
     */
    batchApply(elements, time, options) {
        if (!elements || elements.length === 0)
            return;
        // Performance guardrail (> 200 elements)
        if (elements.length > 200) {
            console.warn(`[keyframe-engine] Performance warning: batchApply bound with ${elements.length} elements (>200 limit). Consider evaluating Canvas or WebGPU rendering path.`);
        }
