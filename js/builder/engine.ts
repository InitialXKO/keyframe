...
export function solveSpringJS(frame: number, fps: number, damping: number, stiffness: number, mass: number): number {
...
export function evaluateEasing(easing: Easing, cubicParams: CubicBezierParams | undefined, t: number): number {
    ...
    if (!this.wasmInstance || (typeof this.wasmInstance.add_clip_json !== "function" && typeof this.wasmInstance.evaluate_frame !== "function")) {
      ...
      const loadPromise = (async () => {
        if (typeof WebAssembly === "undefined") {
          throw new Error(
            "Environment does not support WebAssembly. Host environment must support WebAssembly API to load WASM engine module."
          );
        }

        try {
          let wasmInput: any = url;
          ...
          (kfEngine as any).memory = wasmExports.memory;
  ...
  public getEvaluatedInstances(
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (instData?.clip_id && this.clips.has(instData.clip_id)) {
        ...
        if (kfs.length > 0) {
          const customTracks: Record<string, any> = {};
          if (clip.metadata) {
          ...
          if (kfs.length === 1) {
            if (kfs[0].custom_tracks) Object.assign(customTracks, kfs[0].custom_tracks);
          } else {
            let kfIdx = 0;
