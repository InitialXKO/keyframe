...
function composeTransforms(accum: TransformData, curr: TransformData): TransformData {
  return {
    translation: [
...
export class AnimationStack {
  public id: string;
  private _items: Array<{ clip: Clip | AnimationClipData; options?: StackItemOptions }> = [];

  constructor(id: string) {
    this.id = id;
  }

  public add(clip: Clip | AnimationClipData, options?: StackItemOptions): this {
    this._items.push({ clip, options });
    return this;
  }

  public getItems(): Array<{ clip: Clip | AnimationClipData; options?: StackItemOptions }> {
    return [...this._items];
  }

  public expand(options?: { adaptiveSampling?: boolean; maxErrorThreshold?: number }): EngineIR {
...
export function expandStack(
  source: AnimationStack | EngineIR,
  ...
  let accumulatedTransform: TransformData = {
    translation: [0, 0, 0],
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    if (isDynamic) {
      ...
      const expandedKeyframes: KeyframeData[] = (clipData.keyframes || []).map((kf) => {
      ...
      if (options?.adaptiveSampling && expandedKeyframes.length > 1) {
        ...
        const sampledKfs: KeyframeData[] = [];

        function subdivide(kf1: KeyframeData, kf2: KeyframeData, depth = 0, maxDepth = 4): void {
          ...
          let factor = normT;
          if (kf1.easing === "Ease" as any || kf1.easing === "EaseInOut" as any) {
            factor = 3 * (1 - normT) * normT * normT * 0.58 + normT * normT * normT;
          } else if (kf1.easing === "EaseIn" as any) {
            factor = normT * normT;
          } else if (kf1.easing === "EaseOut" as any) {
          ...
          const err = Math.hypot(
            actualTrans.translation[0] - linearTrans.translation[0],
            actualTrans.translation[1] - linearTrans.translation[1],
            actualTrans.translation[2] - linearTrans.translation[2]
          );
      ...
      const expandedClip: AnimationClipData = {
        ...clipData,
        id: expandedClipId,
